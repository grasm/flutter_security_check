...
public class FlutterSecurityCheckPlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "requestPlayIntegrityToken":
      result(nil) // Not supported on iOS
  ...
  private func getSecurityDetails() -> [String: Any] {
    ...
    if let appStoreReceiptURL = Bundle.main.appStoreReceiptURL {
        ...
        } else if appStoreReceiptURL.path.contains("CoreSimulator") {
            installer = "Simulator"
        } else {
            installer = "AppStore"
        }
    }
    details["installerPackage"] = installer
    
    // Stubs for features not yet fully implemented or applicable on iOS
    details["isMockLocation"] = false
    details["isAccessibilityEnabled"] = UIAccessibility.isVoiceOverRunning
    details["isAppClone"] = false
  ...
  private func isFridaDetected() -> Bool {
    ...
    for i in 0..<count {
      if let imageName = _dyld_get_image_name(i) {
        ...
        if name.contains("FridaGadget") || name.contains("frida") || name.contains("CydiaSubstrate") || name.contains("MobileSubstrate") || name.contains("SSLKillSwitch") {
          return true
        }
      }
    }
